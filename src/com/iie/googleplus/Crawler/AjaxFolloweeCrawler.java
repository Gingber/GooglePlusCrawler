...
public class AjaxFolloweeCrawler extends AbstractCrawler {
	
	private static String baseUrl = "https://plus.google.com/_/socialgraph/lookup/visible/?o=%5Bnull%2Cnull%2C%22$%22%5D&_reqid=1654627&rt=j";
	private DefaultHttpClient httpclient;
	
	public 	AjaxFolloweeCrawler(DefaultHttpClient httpclient, MySQLOperator dbop) {
	...
	public boolean doCrawl(Task task, MulityInsertDataBase dbo, Vector<GPlusUser> RelateUsers, ReportData reportData) {
		LogSys.nodeLogger.debug("Followee Grabber is starting......");
		...
		try {
	        ...
	        for(int i = 0; i < followerInnerArray.length(); i++) {	
	        	...
	        	String followeeName = followerInnerArray.getJSONArray(i).getJSONArray(2).getString(0);
	        	
	        	userrels.add(new UserRelationship(task.getTargetString(), followerId, true+""));
	        	
	        	GPlusUser gpUser = new GPlusUser();
	        	gpUser.setUserId(followerId);
				gpUser.setUserName(followeeName);
