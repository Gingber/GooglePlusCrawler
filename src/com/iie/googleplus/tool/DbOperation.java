...
public class DbOperation {
	...
	public Connection conDB(String ip, String databaseName, String user,String password){
		connectionCount++;
		System.out.println("创建了一个连接，总共的连接数是？？"+connectionCount);
		Connection connect = null;
		try {
			connect = DriverManager.getConnection("jdbc:mysql://" + ip
					+ ":3306/" + databaseName
	...
	public boolean insertWithoutBatch(String insertSql){
		try {
			...
			System.out.println("SQL中含有错误标示符："+insertSql);
		}catch (SQLException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
			System.out.println("错误：Sql语句是："+insertSql);
	...
	public boolean insert(String insertSql) {
		try {
			...
			if(e.getErrorCode()==1062){
				System.out.println("数据重复with SQL("+insertSql+")");
				return false;
			}
			...
			System.out.println("数据重复：SQL语句为："+insertSql);
			return false;
		} catch (SQLException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
